...
class DRGTrainer:
    """
    Treina e avalia o pipeline completo DRG-BR.
    """
    ...
    def _create_dataloader(self, feature_set: FeatureSet, shuffle: bool = True) -> DataLoader:
        """Cria DataLoader a partir de FeatureSet."""
        tensors = [
    ...
    def train_neural(
        ...
        """
        Treina o modelo neural multi-target.
        """
        logger.info("="*60)
        ...
        for epoch in range(self.cfg.epochs):
            ...
            for batch in train_loader:
                cid_p, cids_s, proc, mdc, numeric, targets_r, targets_c = [
    ...
    def train_gbm(
        ...
        """Treina modelos Gradient Boosting."""
        logger.info("="*60)
    ...
    def evaluate(
        ...
        """
        Avalia modelo(s) no conjunto fornecido.
        """
        if model_type == "neural" and self.neural_model:
            ...
            all_preds = {}
            if preds_nn:
                all_preds['neural'] = preds_nn
            if preds_gbm:
                all_preds['gbm'] = preds_gbm
        ...
        results = evaluate_all(
            preds,
            feature_set.targets_regression,
            feature_set.targets_classification,
        )
        return results
    ...
    def train_full_pipeline(
        ...
        """
        Pipeline completo: Neural + GBM + Ensemble.
        """
        results = {}
        ...
        # 2. Treinar GBM
        try:
            gbm_metrics = self.train_gbm(train_set, val_set)
            results['gbm_train'] = gbm_metrics
        ...
        # 3. Ensemble
        if self.neural_model and self.gbm_model:
            ...
            for i, t in enumerate(REGRESSION_TARGETS):
                if i < val_set.targets_regression.shape[1]:
                    true_vals[t] = val_set.targets_regression[:, i]
            for i, t in enumerate(CLASSIFICATION_TARGETS):
                if i < val_set.targets_classification.shape[1]:
                    true_vals[t] = val_set.targets_classification[:, i]

            self.ensemble.optimize_weights(
                {'neural': preds_nn, 'gbm': preds_gbm},
                true_vals
            )
    ...
    def save_models(
        self,
        dir_path: str,
        cid_vocab_size: int = 0,
        sigtap_vocab_size: int = 0,
        n_numeric_features: int = 0,
    ):
        """
        Salva todos os modelos e metadados necessários para reconstrução.

        Arquivos gerados:
        - neural_model.pt         → pesos do modelo neural (state_dict)
        - gbm_model.pkl           → modelos LightGBM/XGBoost (pickle)
        - ensemble_weights.pkl    → pesos otimizados do ensemble
        - model_metadata.json     → metadados para reconstruir a arquitetura
        - training_metrics.json   → métricas finais de treino/val/teste
        """
        ...
        if self.neural_model:
            ...
            logger.info(f"  neural_model.pt salvo ({self.neural_model.count_parameters():,} parâmetros)")

        # 2. GBM
        if self.gbm_model:
            self.gbm_model.save(str(path / "gbm_model.pkl"))
            logger.info(f"  gbm_model.pkl salvo ({len(self.gbm_model.models)} modelos)")

        # 3. Ensemble
        if self.ensemble:
            with open(path / "ensemble_weights.pkl", 'wb') as f:
                pickle.dump(self.ensemble.weights, f)
            logger.info(f"  ensemble_weights.pkl salvo")

        # 4. Metadados de arquitetura (CRÍTICO para reconstrução)
        metadata = {
            'cid_vocab_size': cid_vocab_size,
            'sigtap_vocab_size': sigtap_vocab_size,
        ...
        logger.info(f"  model_metadata.json salvo")

        # 5. Métricas de treino
        if self.best_metrics:
            # Converter numpy types para serialização JSON
            def _sanitize(obj):
            ...
            logger.info(f"  training_metrics.json salvo")
